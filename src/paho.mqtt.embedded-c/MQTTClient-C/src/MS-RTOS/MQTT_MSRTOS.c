...
{
    ms_mutex_create("mqtt_mutex", MS_WAIT_TYPE_PRIO, &mutex->lockid);
}

int MutexLock(Mutex* mutex)
{
    return ms_mutex_lock(mutex->lockid, MS_TIMEOUT_FOREVER);
}

int MutexUnlock(Mutex* mutex)
{
    return ms_mutex_unlock(mutex->lockid);
}

int ThreadStart(Thread* thread, void (*fn)(void*), void* arg)
{
    return ms_thread_create(__MQTT_THREAD_NAME, (ms_thread_entry_t)fn, arg,
                            __MQTT_THREAD_STK_SIZE, __MQTT_THREAD_PRIO, __MQTT_THREAD_TIME_SLICE,
